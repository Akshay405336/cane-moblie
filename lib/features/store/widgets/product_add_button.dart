...
class _ProductAddButtonState extends State<ProductAddButton> {
  ...
  int _getCurrentQty() {
    ...
    if (isLoggedIn) {
      ...
      try {
        final item =
            cart.items.firstWhere((i) => i.productId == productId);
      ...
      try {
        final item =
            items.firstWhere((i) => i.productId == productId);
        return item.quantity;
      } catch (_) {
        return 0;
      }
    ...
    try {
      if (isLoggedIn) {
        ...
        if (newQty == 1 && _getCurrentQty() == 0) {
          ...
          LocalCartController.instance.updateQty(
            product.id,
            newQty,
          );
        }
            ...
            child: Row(
              ...
              children: [
                _IconBtn(
                  icon: Icons.remove,
                  onTap:
                      isLoading ? null : () => _updateQty(qty - 1),
                ),
                SizedBox(
                  width: 20,
                  child: isLoading
